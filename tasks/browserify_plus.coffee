...
module.exports = (grunt) ->
  ...
  grunt.registerMultiTask "browserify_plus", "Continuously Browserify JavaScript and CoffeeScript files fast (it caches)", ->
    ...
    @files.forEach (file) ->
        ...
        watchify.bundle {
          ...
          if err
            ...
            fs.writeFile destPath, src, (err) ->
              console.log "browserify bundle done."
      ...
      watchify.on "update", (id) ->
        console.log "browserify bundle #{id.join(', ')}"
        bundle()

      console.log "---------------------"
      console.log "Grunt-Browserify-Plus"
      console.log "---------------------"
